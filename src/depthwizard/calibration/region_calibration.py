...
def fit_region_calibration(
    ...
    If `base_elevation` is provided, it is subtracted from `reference_elevation`
    before fitting. This anchors the `shift` around zero (relative metric height),
    preventing the fit from hardcoding the source scene's absolute altitude.
    ...
    if base_elevation is not None:
        target_elev = reference_elevation - base_elevation
    else:
        target_elev = reference_elevation

    known_pixel_mask = valid_mask & np.isin(semantic_labels, KNOWN_CLASSES)
    a, b, inlier_frac, r2 = _fit_one_class(
        relative_depth[known_pixel_mask], target_elev[known_pixel_mask]
    )
    ...
    for class_id in KNOWN_CLASSES:
        ...
        a, b, inlier_frac, r2 = _fit_one_class(
            relative_depth[class_mask], target_elev[class_mask]
        )
...
def apply_region_calibration(
    ...
    If `base_elevation` is provided, it is added to the predicted height,
    restoring absolute metric geography if the fit was anchored around zero.
    """
    ...
    if base_elevation is not None:
        absolute_height += base_elevation
